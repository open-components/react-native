...
const storage = new Storage({
    ...
    defaultExpires: null, // 一小时数据过期

    // 读写时在内存中缓存数据。默认启用。
    enableCache: true,

    // 在构造函数这里就写好sync的方法 或是写到另一个文件里，这里require引入 或是在任何时候，直接对storage.sync进行赋值修改
    // sync: require('./sync')
});
...
global.READ_CACHE_ASYNC = readCache_async;
